...
class ConnectFourBoard(object):
    ...
    def __init__(self, board_array = None, board_already_won = None, modified_column = None, current_player = 1, previous_move = -1, win_num = 4):
    ...
    def do_move(self, column):
        ...
        return ConnectFourBoard(new_board, board_already_won=self.is_win(), modified_column=column, current_player = self.get_other_player_id(),win_num=self.win_num)
    ...
    def _is_win_from_cell(self, row, col):
        ...
        return ( self._max_length_from_cell(row, col) >= self.win_num )
    ...
    def clone(self):
        ...
        return ConnectFourBoard(self._board_array, board_already_won=self._is_win, current_player = self.get_current_player_id(),win_num=self.win_num)
...
def run_game(player1, player2, board = ConnectFourBoard(),winNum = 4):
    """ Run a game of Connect Four, with the two specified players """
    board.win_num = winNum
